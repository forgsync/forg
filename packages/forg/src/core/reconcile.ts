...
export async function reconcile(repo: IRepo, forgClient: ForgClientInfo, branchName: string, merge: MergeFunc): Promise<Hash> {
  ...
  const mergeResults = await mergeBase(repo, heads.map(h => h.head.hash));
  ...
  if (leafHeads.length === 0) {
    ...
    if (myHead === undefined || myHead.head.hash !== leafHeads[0].head.hash) {
      ...
      await updateRef(repo, myRef, mergeResults.leafCommitIds[0], createCommitterInfo(forgClient), `reconcile (fast-forward): ${leafHeads[0].clientUuid}`);
    }
  ...
  leafHeads.sort((a, b) => a.head.commit.body.committer.date.seconds - b.head.commit.body.committer.date.seconds);
  ...
  await updateRef(repo, myRef, prev, createCommitterInfo(forgClient), `reconcile: ${leafHeads.map(h => h.clientUuid).join(', ')}`);
