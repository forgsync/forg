...
import { ForgClientInfo, SyncConsistencyOptions } from "./model";
 ...
 */
export async function fetchRefs(origin: IReadOnlyRepo, local: IRepo, client: ForgClientInfo, consistency?: SyncConsistencyOptions): Promise<void> {
 ...
 */
export async function fetchRef(origin: IReadOnlyRepo, local: IRepo, ref: string, consistency?: SyncConsistencyOptions): Promise<Hash | undefined> {
  ...
  if (remoteRefHash) {
    try {
      const commit = await syncCommit(origin, local, remoteRefHash, consistency);
      fetchedHead = {
        oid: remoteRefHash,
        commit,
      };
  ...
  if (fetchedHead === undefined) {
    for (let i = remoteReflog.length - 1; i >= 0; i--) {
      ...
      try {
        const commit = await syncCommit(origin, local, reflogEntry.newCommit, consistency);
        fetchedHead = {
