...
describe('Path', () => {
  test.each(['', '/'])(
    "'%p' is a valid root", (input: string) => {
  ...
  test.each(['ab c/d', '/ab c/d', 'ab c/d/'])(
    "Two segments '%p'", (input: string) => {
      const path = new Path(input);
  ...
  test.each(['//', '/a//b', '/a\\b/c', 'a/b\tc/d'])(
    "Rejects suspicious '%p'", (input: string) => {
